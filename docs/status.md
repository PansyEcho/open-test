...
`codex-first-system-workflows-and-qa-tools` 已完成实现与真实QA验收：知识准备沿用原流程并只修复缺失batch覆盖原异常的问题；显式`$open-test`和系统Skill已覆盖系统管理、受控源码、多接口顺序知识、接口Case、Case执行以及Facade、Job、外部DSF、MQ和数据库统一操作。`capability-driven-operation-resolution`、`codex-native-open-test-workspace`和`semantic-knowledge-workspace`既有结论保持不变；`dsf-execution-and-oracles`保持`WAITING_QA_INPUT`。
...
- 2026-08-25：Codex优先系统工作流与统一QA操作已闭合。知识准备没有新增`PREPARING`状态、阶段状态机或额外持久化：`_submit_codex_client_handoff`在batch不存在时直接保留并记录原始prepare异常，在batch已存在时继续既有恢复；Skill路径若只缺task，则用预分配ID补齐同一个task，不重新准备batch或创建线程；页面对未分类异常返回移除凭据/连接后的真实异常类型和消息。当前scan `scan-20260825075610-a0f437c374-8132c1a1`的真实`RefundFacade#createOrder`页面入口已成功得到batch `knowledge-client-ebe9c35397044be5`、task `task-73e404d2f79e7a8f`、handoff `handoff-f44a428898054d3ba5e92f7a`和Codex深链线程`01a037ad-c8db-7082-9d04-3c5ec73543ed`，因此历史`knowledge draft batch not found`故障以真实入口成功闭合；历史最初异常未留存，未虚构业务根因。插件`open-test-knowledge`已刷新为`0.2.0+codex.20260825080000`，源目录与安装缓存中的`$open-test`、`$open-test-ifightchainsaas-java-refund-core`均通过Skill校验，安装态MCP已通过`127.0.0.1:8788`成功列出当前系统。接口Case在知识不完整时已保存，并以`operation_id`执行真实READ Case：generation `case-generation:acf27ab8c46741347cb6`、batch `case-batch:9f5d4ff552bf768513b5`、snapshot `snapshot-42d5b6ab97cfdf1937a64a9f`、run `run-2aab291e721c4f7a9aff`，2步均通过。真实QA验收均使用当前扫描和固定`env=qa / targetenv=test`：外部`TradeFacade#queryList` execution `operation-execution-9eddd6ac127d413ca791`查询到2026-08-01后`TICKETED(4)`订单7条；Refund MQ execution `operation-execution-69fb8346c0f7494b9890`返回`SEND_OK`、message ID `0AB136DE00002A9F000015719CF44D4C`和offset `163`；`saas_refund_order_psi`唯一软删除记录`OPENTEST_DB_20260825_1602_02`的INSERT、UPDATE、SELECT executions分别为`operation-execution-af49dba8de434d1591c3`、`operation-execution-53bcf828da4c4f8ba03e`、`operation-execution-bd768952d5cc4effa2b0`，两次写均提交1行且查询返回1行。业务结果和真实业务错误现完整持久化，只有Token、配置凭据和连接地址被移除；Job非零退出也保留已有结构化业务输出。最终门禁为V2 `449 passed / 1 skipped`、legacy `43 passed`、DSF Worker `6 tests / 0 failures`、Java Semantic Sidecar `12 tests / 0 failures`、QA Oracle Worker `49 tests / 0 failures`、OpenSpec strict `35 passed / 0 failed`，compileall、Node、`pip check`、Skill源/安装态、插件桥接和diff检查通过；最终代码已重启于`127.0.0.1:8788`并返回HTTP 200。OCR delegation由同一个只读代理完成两遍：初审`1 High / 4 Medium / 1 Low`全部接受并修复；复审确认原问题闭合并新增`0 High / 1 Medium`，指出MCP读取本机设置响应键错误，已改为真实`local_settings`并用FastAPI响应形状回归闭合；按两轮上限未启动第三遍，无拒绝项。`RefundFacade#queryList`知识没有重新生成或修改，发布文件SHA-256仍为`13ada1181d643138fab239d4b4742a182121bd89ac213988658e2e89a5f31aa1`。
- 2026-08-24：Codex系统Skill真实QA退票查询故障已闭合。根因为Refund.Core源码同时声明`dsf.service.config.env=qa`与`dsf.service.config.targetenv=test`，而旧OpenTest Profile只保存并写出`targetenv`；DSF Client 2.5.11实际使用`env`拼接QA服务组，因而旧执行查找了无`qa.`前缀的组并抛出被包装的`ServiceNotFoundException`。当前Profile独立保存两项源码证据，Python与Java Worker双层QA门禁分别写出两个属性；干净历史扫描从Manifest记录的Git commit补导缺失`env`，dirty/non-Git扫描仅在路径、commit、dirty状态与内容摘要完全一致时补导，历史Manifest不改写。Worker失败现会令外层execution进入`failed`并保留脱敏错误分类，相同`request_id`不重复派发；`ServiceNotFoundException`和timeout的cause链分类已修正。经用户明确授权，只执行了一次真实QA只读`RefundFacade#queryList`：execution `operation-execution-794aa0f2b72747c085b0`为外层`completed`、Worker `success`、业务`success=true`，耗时`5678 ms`，返回`2 / total 2 / 1页`且两行均有非空退票单号；未记录于本文或对话输出具体单号和完整响应，未执行WRITE或Job。审查修复后的最终门禁为定向Python `34 passed`、V2 `438 passed / 1 skipped`、legacy `43 passed`、DSF Worker `6 tests / 0 failures`、Java Semantic Sidecar `12 tests / 0 failures`、QA Oracle Worker `45 tests / 0 failures`、OpenSpec strict `34 passed / 0 failed`，compileall、Node、`pip check`、插件覆盖和diff检查通过；本机`127.0.0.1:8788`已用最终代码重启并返回HTTP 200。OCR delegation严格只审本次路由缺陷隔离补丁：初审`1 High / 1 Medium / 1 Low`全部修复，唯一复审`0 High / 1 Medium / 0 Low`指出Git tree物化路径边界并已补绝对路径、父级跳转和最终归属校验；按两轮上限未启动第三轮，无拒绝项。`queryList`知识生成未重跑，原任务/线程不变，Manifest SHA-256仍为`4decaf58b46771932c9558923d144f937f1586d5989cf39ed3da5493b6832337`，发布知识SHA-256仍为`13ada1181d643138fab239d4b4742a182121bd89ac213988658e2e89a5f31aa1`；未读取Fixture正文、未运行生命周期Case。
- 2026-08-24：证据型操作契约与声明式能力改造完成。Java Sidecar v4分别输出字段Javadoc、文档`@required`、已知运行时校验注解和字面量初始化证据；当前39个操作已由不可变历史扫描派生为`operation-capability/v2`，未写回Manifest。真实`RefundFacade#queryList`契约为`required_fields=[]`且不含`safe_defaults`：`page=1`、`pageSize=20`仅为`declared_initializer`，`platFormId`为文档标记但非运行时必填；合成票号原始问法稳定首选该READ操作，fake provider仅收到`ticketNo`且相同`request_id`只调用一次。通用代码词表不含退票、票号或订单业务名词，系统/Facade/Job差异来自Profile、源码字段和知识证据；旧未配置系统只可复用退役场景模板，不能获得专用追踪、Worker或真实执行授权。系统Skill和operations MCP已同步，插件版本为`0.1.0+codex.20260824103208`并通过源目录、安装目录双重校验，新Codex任务加载该版本。最终门禁：V2 `432 passed / 1 skipped`、legacy `43 passed`、Java语义Sidecar `12 tests / 0 failures`、QA Oracle Worker `45 tests / 0 failures`、OpenSpec strict `34 passed / 0 failed`，两个Java模块package、compileall、Node语法、`pip check`、Skill/插件和diff检查通过。OCR delegation preview/rule成功解析本次干净HEAD上的`27 reviewable / 39 total`范围；只读代理两种隔离方式都在创建前被当前桌面任务残留的旧插件缓存路径阻断，未产生代理实例或审查结果，因此按规范执行两遍完整主代理fallback审查：初审`0 High / 4 Medium / 2 Low`，修复嵌套DTO约束误抬高父对象、Profile场景fallback泄漏、完整语义对象无界常驻、Worker应用身份隐式回退，以及未使用诊断函数和重复Profile ID门禁；唯一follow-up为`0 High / 0 Medium / 0 Low`，无拒绝项。`queryList`知识任务与聊天未重跑，任务`task-c0b2de4673b01efa`、线程`01a03270-708f-79d1-80a6-62491ecb863d`不变，Manifest SHA-256保持`4decaf58b46771932c9558923d144f937f1586d5989cf39ed3da5493b6832337`，发布知识SHA-256保持`13ada1181d643138fab239d4b4742a182121bd89ac213988658e2e89a5f31aa1`。未访问QA或Fixture正文，未执行DSF金丝雀、真实WRITE/Job或31个生命周期Case。
