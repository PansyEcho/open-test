...
`capability-driven-operation-resolution` 已完成实现：当前可执行操作使用证据型`operation-capability/v2`，源码初始化值不再作为平台默认值，运行时必填只来自可解析的校验框架约束；等价READ由显式系统Skill结合输入/输出语义自行选择一个。系统别名、Job扫描、Worker身份和旧兼容工作流已迁移到受审Capability Profile，旧场景fallback与知识追踪、Worker、真实MVP和通用执行隔离。`codex-native-open-test-workspace`保持完成，已完成的`semantic-knowledge-workspace`及其历史任务结论保持不变；`dsf-execution-and-oracles`保持`WAITING_QA_INPUT`。
