...
- **THEN** 系统必须在prepare handoff完成前沿高置信已解析调用边遍历该入口真实可达的方法，并从方法正文中屏蔽注释和字符串后识别精确`setState(..., XxxStateEnum.STATE, ...)`
...
系统 SHALL 要求Facade候选覆盖业务目的、场景、输入输出语义、日期分页、真实边界路径、重要分支、失败处理、返回与测试Oracle。完整度章节 SHALL 只拒绝空白、明确占位和复制或高度重复内容，不得使用字符数门槛拒绝简短但明确的事实。机器可修复缺口 SHALL 作为结构化`completion_gaps`返回同一task与handoff并保持可修订，当前原生Agent SHALL 能继续补读和提交；OpenTest SHALL NOT 因一次提交、重复缺口或仍在等待用户回答而发布失败产物或启动另一个Agent。
...
#### Scenario: 连续改进复用原业务任务

- **WHEN** 当前原生Agent根据校验结果连续提交仍在改进的候选
- **THEN** 系统保持原task、handoff、batch和scan，返回最新缺口并允许继续修订
- **AND** 不要求原thread存在，也不创建下一次业务任务

#### Scenario: 重复缺口不冒充终态

- **WHEN** 新修订仍得到与前次相同的候选缺口，或任务正在等待持久业务问题的回答
- **THEN** 系统在原任务保留草稿、问题和诊断，不得自动发布`BLOCKED`或宣称业务完成
- **AND** 工程存储错误与业务语义问题使用不同诊断，不能要求Agent通过猜测内容绕过
